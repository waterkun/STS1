"""
Parse all Defect .run files and build a decision database for the advisor.
...
# Defect starting deck: 4x Strike_B, 4x Defend_B, Zap, Dualcast
DEFECT_BASE_DECK = ["Strike_B"] * 4 + ["Defend_B"] * 4 + ["Zap", "Dualcast"]
...
def load_defect_runs() -> list[dict]:
    runs = []
    for root, _, files in os.walk(RUNS_DIR):
        for filename in files:
            ...
            try:
                ...
                if data.get("character_chosen") == "DEFECT":
                    runs.append(data)
...
def build_campfire_decisions(runs: list[dict]) -> dict:
    counters: dict[str, list[int]] = {"REST": [0, 0], "SMITH": [0, 0]}
    hp_buckets = {
    ...
    stats = {
        ...
        "common_upgrades": {
            card: {"total": counts[1], "wins": counts[0],
...
def main():
    print("Loading Defect runs...")
    runs = load_defect_runs()
    wins = sum(1 for r in runs if r["victory"])
    print(f"Loaded {len(runs)} runs ({wins} wins, {len(runs)-wins} losses)")

    if len(runs) == 0:
        print("No Defect runs found. Make sure .run files are in the runs/ directory.")
        return
