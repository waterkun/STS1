#!/usr/bin/env python3
"""
Defect ML Advisor V4 - A20 胜局专用 Transformer
...
Usage:
  ...
  python -m defect_advisor.ml_advisor_v4 card \\
    --floor 8 --act 1 --hp 45 --max-hp 80 \\
    --relics "Cracked Core" \\
    --deck "Strike_B x4,Defend_B x4,Zap,Dualcast" \\
    --options "Compile Driver,Claw,Coolheaded"
"""
...
def main():
    parser = argparse.ArgumentParser(description="机械师 ML 决策顾问 V4 (A20 胜局 Transformer)")
    ...
    if args.command == "card":
        ...
        preds = predict_all_boss_relic(options, args.act, hp_pct,
                                       deck, relics, db, vocab, v4_models)
        print(f"\n=== 机械师 V4 Transformer 决策建议 (Boss 遗物选择) ===")
