#!/usr/bin/env python3
"""
Ironclad ML Advisor - XGBoost/LightGBM 决策预测模型

基于历史对局数据训练分类器，预测每个选项的胜率。

Usage:
  # 训练所有模型
  python ironclad_advisor/ml_advisor.py train

  # 卡牌决策
  python ironclad_advisor/ml_advisor.py card \
  ...
  # 篝火决策
  python ironclad_advisor/ml_advisor.py campfire \
    --floor 11 --act 1 --hp 35 --max-hp 80 \
    --relics "Burning Blood" \
    --deck "Strike_R x4,Defend_R x4,Bash,Iron Wave"

  # Boss 遗物
  python ironclad_advisor/ml_advisor.py boss-relic \
    --act 1 --hp 55 --max-hp 80 \
    --relics "Burning Blood" \
    --deck "Strike_R x4,Defend_R x4,Bash,Barricade" \
    --options "Snecko Eye,Cursed Key,Coffee Dripper"

  # 商店决策
  python ironclad_advisor/ml_advisor.py shop \
    --floor 27 --act 2 --hp 60 --max-hp 80 --gold 300 \
    --relics "Burning Blood,Snecko Eye" \
    --deck "Strike_R x3,Defend_R x4,Bash,Barricade" \
...
def load_db() -> dict:
    ...
    for name in ["card_decisions", "boss_relic_decisions", "campfire_decisions", "shop_decisions"]:
        ...
        if not path.exists():
            ...
            print("请先运行: python ironclad_advisor/build_db.py")
            sys.exit(1)
...
def base_features(floor: int, act: int, hp_pct: int, deck_size: int,
    ...
    """通用特征向量：数值特征 + act one-hot + 卡组编码 + 遗物编码 + 升级特征。"""
...
def build_card_features(db: dict, vocab: dict) -> tuple[np.ndarray, np.ndarray]:
    """为每条卡牌决策生成特征：游戏状态 + 被选中的卡牌，标签为胜负。

    每条决策只产生一行（被选中的选项），模型学习：
    给定游戏状态 + 选择的卡 → 预测胜率。
    推理时对每个候选选项分别预测。
    """
...
def build_campfire_features(db: dict, vocab: dict) -> tuple[np.ndarray, np.ndarray]:
    """为每条篝火决策生成一行特征（使用实际选择），标签为该局胜负。

    模型学习：给定游戏状态 + 选择(REST/SMITH) → 预测胜率。
    推理时对两种选择分别预测。
    """
    rows = []
    labels = []
...
def campfire_inference_features(floor: int, act: int, hp_pct: int,
    ...
    rows = []
    for choice_code in [0.0, 1.0]:  # REST, SMITH
        extra = np.array([choice_code,
                          hp_below_30, hp_30_50, hp_50_70, hp_above_70],
                         dtype=np.float32)
        rows.append(np.concatenate([base, extra]))
...
def build_boss_relic_features(db: dict, vocab: dict) -> tuple[np.ndarray, np.ndarray]:
    """为每条 Boss 遗物决策生成一行特征（使用实际选择的遗物）。

    模型学习：给定游戏状态 + 选择的遗物 → 预测胜率。
    """
...
def build_shop_features(db: dict, vocab: dict) -> tuple[np.ndarray, np.ndarray]:
    """为每个商店访问中的每个可用物品生成一行特征。

    每行 = 游戏状态 + 金币 + 该物品编码 + 该物品统计 + 是否购买。
    模型学习：给定游戏状态 + 一个具体物品 → 预测胜率。
    推理时对每个物品分别打分。
    """
...
def train_model(X: np.ndarray, y: np.ndarray, model_name: str,
    ...
    """用 5-fold CV 训练 XGBoost 和 LightGBM，返回模型和指标。"""
    results = {}
    n_folds = 5
    # 确保至少有两个类
...
def load_models(name: str) -> dict:
    ...
    if not result:
        print(f"未找到 {name} 模型，请先运行: python ironclad_advisor/ml_advisor.py train")
        sys.exit(1)
    return result
...
def load_vocab() -> dict:
    ...
    if not vocab_path.exists():
        print(f"未找到词表文件，请先运行: python ironclad_advisor/ml_advisor.py train")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="铁甲战士 ML 决策顾问 (XGBoost/LightGBM)",
