...
def print_predictions(options: list[str], preds: dict,
    ...
    Args:
        options: 候选项名称列表
        preds: {engine_name: np.ndarray} 各模型预测
        prices: 每个选项的价格（仅商店），None 表示非商店
        gold: 玩家当前金币
    """
...
def main():
    ...
    while True:
        ...
        if not msg.get("in_game"):
            # Reset boss tracking on game exit
            act1_boss = ""
            act2_boss = ""
            _last_act = 0
            send("wait")
        ...
        if character and character != "IRONCLAD":
            key = ("non_ironclad", character)
            if key != last_advice_key:
                log(f"当前角色: {character}，模型仅支持 IRONCLAD，跳过建议。")
                last_advice_key = key
            send("wait")
            continue
        ...
        # Track boss names across act transitions
        # gs["boss"] shows the upcoming/current boss for the current act
