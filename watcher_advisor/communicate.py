#!/usr/bin/env python3
"""
CommunicationMod 透传建议模式 - Watcher
"""
...
LOG_PATH = os.path.join(_project_root, "watcher_advisor.log")

logger = logging.getLogger("watcher_advisor")
...
try:
    ...
    from watcher_advisor.ml_advisor_v2 import (
        load_v2_models,
        predict_all_card,
...
try:
    ...
    from watcher_advisor.ml_advisor_v3 import (
        load_v3_models,
...
try:
    ...
    from watcher_advisor.ml_advisor_v4 import (
        load_v4_models,
...
def main():
    ...
    while True:
        ...
        if character and character != "WATCHER":
            key = ("non_watcher", character)
            if key != last_advice_key:
                log(f"当前角色: {character}，模型仅支持 WATCHER，跳过建议。")
                last_advice_key = key
            send("wait")
            continue
