#!/usr/bin/env python3
"""
Silent ML Advisor - XGBoost/LightGBM 决策预测模型
...
Usage:
  ...
  # 卡牌决策
  python -m silent_advisor.ml_advisor card \
    --floor 8 --act 1 --hp 45 --max-hp 70 \
    --relics "Ring of the Snake" \
    --deck "Strike_G x5,Defend_G x4,Survivor,Neutralize" \
    --options "Deadly Poison,Backflip,Blade Dance"

  # 篝火决策
  python -m silent_advisor.ml_advisor campfire \
    --floor 11 --act 1 --hp 35 --max-hp 70 \
    --relics "Ring of the Snake" \
    --deck "Strike_G x5,Defend_G x4,Survivor,Neutralize,Backflip"

  # Boss 遗物
  python -m silent_advisor.ml_advisor boss-relic \
    --act 1 --hp 55 --max-hp 70 \
    --relics "Ring of the Snake" \
    --deck "Strike_G x5,Defend_G x4,Survivor,Neutralize,Noxious Fumes" \
    --options "Snecko Eye,Cursed Key,Coffee Dripper"

  # 商店决策
  python -m silent_advisor.ml_advisor shop \
    --floor 27 --act 2 --hp 50 --max-hp 70 --gold 300 \
    --relics "Ring of the Snake,Snecko Eye" \
    --deck "Strike_G x3,Defend_G x4,Survivor,Neutralize,Noxious Fumes" \
    --cards "Wraith Form,Catalyst,Footwork" \
    --shop-relics "Mark of Pain,Du-Vu Doll" \
    --potions "Poison Potion,GhostPotion"
"""
...
def load_db() -> dict:
    ...
    for name in ["card_decisions", "boss_relic_decisions", "campfire_decisions", "shop_decisions"]:
        ...
        if not path.exists():
            ...
            print("请先运行: python silent_advisor/build_db.py")
            sys.exit(1)
...
def load_models(name: str) -> dict:
    ...
    if not result:
        print(f"未找到 {name} 模型，请先运行: python -m silent_advisor.ml_advisor train")
        sys.exit(1)
    return result
...
def load_vocab() -> dict:
    ...
    if not vocab_path.exists():
        print(f"未找到词表文件，请先运行: python -m silent_advisor.ml_advisor train")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="静默猎人 ML 决策顾问 (XGBoost/LightGBM)",
