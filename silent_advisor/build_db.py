"""
Parse all Silent .run files and build a decision database for the advisor.

Run once:
    python silent_advisor/build_db.py
"""
...
SILENT_BASE_DECK = ["Strike_G"] * 5 + ["Defend_G"] * 4 + ["Survivor", "Neutralize"]
...
def load_silent_runs() -> list[dict]:
    runs = []
    for root, _, files in os.walk(RUNS_DIR):
        for filename in files:
            ...
            try:
                ...
                if data.get("character_chosen") == "THE_SILENT":
                    runs.append(data)
...
def main():
    print("Loading Silent runs...")
    runs = load_silent_runs()
